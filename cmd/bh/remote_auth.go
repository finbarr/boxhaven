...
func remoteAuthPollDeviceToken(backendURL string, device remoteAuthDeviceCodeResponse) (remoteAuthDeviceTokenResponse, error) {
	...
	for {
		if time.Now().After(deadline) {
			return remoteAuthDeviceTokenResponse{}, fmt.Errorf("browser login expired; run `bh login` again")
		}
		...
		case "slow_down":
			...
			continue
		case "expired_token":
			return token, fmt.Errorf("browser login expired; run `bh login` again")
		case "access_denied":
