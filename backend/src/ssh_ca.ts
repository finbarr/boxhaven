...
export class SSHCertificateAuthority {
  ...
  async signUserPublicKey(request: {
    ...
    const identity = sanitizeIdentity(request.identity || "boxhaven");
    ...
    const dir = await mkdtemp(join(tmpdir(), "boxhaven-ssh-cert-"));
    try {
...
function sanitizeIdentity(value: string): string {
  return value.trim().replace(/[^A-Za-z0-9._:-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 128) || `boxhaven-${randomUUID()}`;
}
