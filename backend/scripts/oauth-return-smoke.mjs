...
try {
  ...
  for (const [scenario, destination, action] of [
    ...
    // Starting from a failed email-verification return must not replay that
    // result after OAuth brings the user back to the original request.
    await page.goto(`${appURL}${destination}${scenario === "device" ? "&verified=true&error=TOKEN_EXPIRED" : ""}`);
