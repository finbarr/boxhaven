...
ssh "${ssh_opts[@]}" "$ssh_target" \
  "test -f /opt/boxhaven/remote/ready && command -v codex >/dev/null && command -v claude >/dev/null && command -v docker >/dev/null && command -v rsync >/dev/null && test -x /usr/local/bin/boxhaven-remote-session && test -x /usr/local/lib/boxhaven/agent.mjs && systemctl list-unit-files boxhaven-agent.service >/dev/null"
