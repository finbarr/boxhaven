...
Build a DigitalOcean snapshot that already contains the BoxHaven VM
...
SSH key selection:
  ...
  script registers BOXHAVEN_IMAGE_BUILDER_SSH_PUBLIC_KEY, the first ssh-agent key,
...
env_file="${BOXHAVEN_REMOTE_IMAGE_ENV_FILE:-${repo_root}/deploy/digitalocean/.env.production}"
arg_ref=""
...
image_ref="${arg_ref:-${BOXHAVEN_REMOTE_IMAGE_REF:-}}"
source_dir="$(cd "$source_dir" && pwd)"
timestamp="$(date -u +%Y%m%d%H%M%S)"
builder_name="boxhaven-image-builder-${timestamp}"
...
first_public_key() {
  if [ -n "${BOXHAVEN_IMAGE_BUILDER_SSH_PUBLIC_KEY:-}" ]; then
    printf '%s\n' "$BOXHAVEN_IMAGE_BUILDER_SSH_PUBLIC_KEY"
    return 0
  fi
...
if [ -n "$image_ref" ]; then
  ...
  archive_path="${tmp_dir}/boxhaven-source.tgz"
...
tags_json="$(json_csv_array "${DIGITALOCEAN_TAGS:-boxhaven}")"
tags_json="$(jq -cn --argjson configured "$tags_json" '$configured + ["boxhaven-image-builder"] | unique')"
...
ssh "${ssh_opts[@]}" "$ssh_target" \
  "test -f /opt/boxhaven/remote/ready && command -v codex >/dev/null && command -v claude >/dev/null && command -v docker >/dev/null && command -v rsync >/dev/null && test -x /usr/local/bin/boxhaven-remote-session && test -x /usr/local/lib/boxhaven/agent.mjs && test \"\$(tmux show-options -gqv mouse)\" = \"on\" && git config --system --get-all safe.directory | grep -Fx /opt/boxhaven/project >/dev/null && systemctl list-unit-files boxhaven-agent.service >/dev/null"
...
if [ "$set_active" -eq 1 ]; then
  ...
  set_env_var "$env_file" "BOXHAVEN_REMOTE_IMAGE" "$snapshot_id"
  set_env_var "$env_file" "BOXHAVEN_REMOTE_IMAGE_REF" "$build_label"
  set_env_var "$env_file" "BOXHAVEN_REMOTE_IMAGE_NAME" "$snapshot_name"
...
Set BOXHAVEN_REMOTE_IMAGE=${snapshot_id} in the backend environment, then restart
