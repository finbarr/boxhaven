...
const { parseMachines, validName, terminalSize, runCLI, cliEnvironment, previewURL } = require('../src/cli.cjs');
...
test('preview links permit web URLs only, without embedded credentials or control characters', () => {
  assert.equal(previewURL('https://work.example.com/path?x=1'), 'https://work.example.com/path?x=1');
  assert.equal(previewURL('http://localhost:8080'), 'http://localhost:8080/');
  for (const value of [null, {}, '', 'file:///tmp/work', 'javascript:alert(1)', 'vscode://work', 'https://user:password@example.com', 'https://example.com/\npath']) assert.equal(previewURL(value), '');
  const [box] = parseMachines(JSON.stringify({ machines: [{ name: 'work', status: 'online', preview_url: 'https://work.example.com' }] }));
  assert.equal(box.previewURL, 'https://work.example.com/');
});
